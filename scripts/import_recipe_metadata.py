...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--schema",
        type=Path,
        default=root / "sql/recipe_eval_schema.sql",
...
def read_and_validate_source(path: Path) -> tuple[list[dict[str, str]], dict[str, dict]]:
    ...
    for row in rows:
        ...
        has_subrecipe = parse_bool(row["has_subrecipe_references"], "has_subrecipe_references", recipe_id)
        is_complete = parse_bool(row["ingredient_list_complete"], "ingredient_list_complete", recipe_id)
        ...
        for tier, keys in tiers.items():
            actual = [record.get("ingredient_key") for record in records if record.get("tier") == tier]
            if actual != keys:
        ...
        for dimension, (_, column, value_kind) in RATING_DIMENSIONS.items():
            ...
            if dimension == "ingredient_complexity" and score != complexity_score(ingredient_count):
                raise ValidationError(f"{recipe_id}: ingredient complexity does not match its fixed bin")
            ratings[dimension] = {
                "score": score,
                "column": column,
...
def check_expected_source_counts(rows: list[dict[str, str]], parsed: dict[str, dict], args: argparse.Namespace) -> None:
    actual = {
...
def insert_all(
    ...
    connection.executemany(
        "INSERT INTO cookbooks(cookbook_id, title_exact, author, rights_context) VALUES (?, ?, ?, ?)",
    ...
    for row in rows:
        ...
        for source_kind, values in (("image", item["images"]), ("page", item["pages"])):
            connection.executemany(
                "INSERT INTO recipe_sources(recipe_id, source_kind, position, source_value) VALUES (?, ?, ?, ?)",
                [(recipe_id, source_kind, position, str(value)) for position, value in enumerate(values, 1)],
            )
        connection.executemany(
            "INSERT INTO recipe_sections(recipe_id, position, heading_exact) VALUES (?, ?, ?)",
            [(recipe_id, position, heading) for position, heading in enumerate(item["sections"], 1)],
        )
        connection.executemany(
            """
        ...
        for dimension, rating in item["ratings"].items():
            connection.execute(
                ...
                (
                    ...
                    rating["score"],
                    rubric_version,
                    "derived" if rating["value_kind"] == "derived" else "provisional_human_annotation",
...
def validate_database(connection: sqlite3.Connection, args: argparse.Namespace) -> dict[str, int]:
    ...
    incomplete = connection.execute(
        "SELECT COUNT(*) FROM recipes WHERE ingredient_list_complete = 0 AND transcription_status = 'partial'"
    ).fetchone()[0]
    ...
    if incomplete != 1 or inconsistent_completeness:
        raise ValidationError(
            f"expected one consistent partial recipe; got partial={incomplete}, inconsistent={inconsistent_completeness}"
        )
